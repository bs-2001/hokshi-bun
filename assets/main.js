...
(function () {
  ...
  function buildSwatch(scheme, activeId) {
    ...
    bar.innerHTML =
      ...
      '<span style="background:' + scheme.darkBg + '"></span>';
    div.appendChild(bar);
  ...
  var pickerGrid = document.getElementById('scheme-picker-grid');
  ...
  if (pickerGrid) {
    SCHEMES.forEach(function (s) {
      pickerGrid.appendChild(buildSwatch(s, activeId));
    });
  }

  // --- Popup toggle (desktop) ---
  ...
  // --- Inline picker toggle (mobile drawer) ---
  var schemeToggleDrawer = document.getElementById('scheme-toggle-drawer');
  var schemePicker = document.getElementById('scheme-picker');

  if (schemeToggleDrawer && schemePicker) {
    schemeToggleDrawer.addEventListener('click', function (e) {
      e.stopPropagation();
      schemePicker.classList.toggle('active');
    });
  }
  ...
  function closeAllPopups() {
    ...
    if (schemePicker) schemePicker.classList.remove('active');
  }
  ...
  document.addEventListener('click', function (e) {
    if (schemePopup && schemePopup.classList.contains('active')) {
      ...
      if (!schemePopup.contains(e.target) && e.target !== schemeToggle && !schemeToggle.contains(e.target)) {
    ...
    if (headings.length > 0) {
      ...
      function buildTree(nodes) {
        ...
        for (var i = 0; i < nodes.length; i++) {
          ...
          var node = { level: level, id: nodes[i].id, text: nodes[i].textContent, el: nodes[i], children: [] };
